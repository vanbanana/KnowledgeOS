...
use crate::services::snapshot::{create_file_snapshot, create_record_snapshot, snapshot_document};
...
pub fn confirm_and_execute(app_state: &AppState, task_id: &str) -> Result<AgentTaskRecord, String> {
    let task =
    ...
    for step in &plan.steps {
        let args: Value =
            serde_json::from_str(&step.arguments_json).map_err(|error| error.to_string())?;
        ...
        if let Err(error) = result {
            append_task_log(
                ...
                task_id,
                "error",
                &format!("步骤 {} 执行失败：{}", step.title, error),
            )?;
...
pub fn rollback_agent_task(app_state: &AppState, task_id: &str) -> Result<AgentTaskRecord, String> {
...
fn execute_step(
    ...
    match tool_name {
        ...
        "read_document" => {
            execute_read_document(app_state, project_id, project_root, args, context)
        }
        ...
        "update_markdown" => {
            execute_update_markdown(app_state, task_id, project_root, args, context)
        }
...
fn execute_rename_file(
    ...
    let document_id = args
        ...
        .ok_or_else(|| "缺少 documentId".to_string())?;
    let new_name = args
        .get("newName")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 newName".to_string())?;
    ...
    let target = resolve_project_relative_path(
...
fn execute_delete_file(
    ...
    if let Some(document) =
        resolve_document_for_execution(app_state, project_id, project_root, args)?
    {
...
fn snapshot_document_with_related_state(
    ...
    let blocks =
        list_blocks(&app_state.db, &document.document_id).map_err(|error| error.to_string())?;
    for block in blocks {
...
fn execute_update_markdown(
    ...
    let content_md = if args.get("contentMode").and_then(Value::as_str) == Some("generate") {
        ...
        let resolved_source_document =
            resolve_document_for_execution(app_state, &task.project_id, project_root, args)?;
    ...
    let relative_path = args
        .get("path")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 blockId 或 path".to_string())?;
    let target = resolve_project_relative_path(Path::new(project_root), relative_path)?;
...
fn execute_merge_cards(app_state: &AppState, task_id: &str, args: &Value) -> Result<(), String> {
    let source_card_id = args
        .get("sourceCardId")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 sourceCardId".to_string())?;
    let target_card_id = args
        .get("targetCardId")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 targetCardId".to_string())?;
    let source_card =
        get_card(&app_state.db, source_card_id)?.ok_or_else(|| "源卡片不存在".to_string())?;
    let target_card =
        get_card(&app_state.db, target_card_id)?.ok_or_else(|| "目标卡片不存在".to_string())?;
    create_record_snapshot(
        &app_state.db,
        task_id,
        "card",
...
fn execute_update_tags(app_state: &AppState, task_id: &str, args: &Value) -> Result<(), String> {
    let card_id = args
        .get("cardId")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 cardId".to_string())?;
    let tags = args
        .get("tags")
...
fn execute_create_relation(
    ...
    let from_node_id = args
        .get("fromNodeId")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 fromNodeId".to_string())?;
    let to_node_id = args
        .get("toNodeId")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 toNodeId".to_string())?;
    let relation_type = args
        .get("relationType")
        .and_then(Value::as_str)
        .unwrap_or("related");
    let snapshot = create_record_snapshot::<Value>(
        &app_state.db,
        task_id,
        "relation",
        &format!("new:{from_node_id}:{to_node_id}:{relation_type}"),
        None,
    )?;
...
fn execute_remove_relation(
    app_state: &AppState,
    task_id: &str,
    args: &Value,
) -> Result<(), String> {
    let relation_id = args
        .get("relationId")
        .and_then(Value::as_str)
        .ok_or_else(|| "缺少 relationId".to_string())?;
    let relation =
        get_relation(&app_state.db, relation_id)?.ok_or_else(|| "关系不存在".to_string())?;
    create_record_snapshot(
        &app_state.db,
        task_id,
        "relation",
...
fn execute_export_project(
    ...
    let project_id = get_agent_task(&app_state.db, task_id)?
        .ok_or_else(|| "Agent 任务不存在".to_string())?
        .project_id;
    let documents =
        list_documents(&app_state.db, &project_id).map_err(|error| error.to_string())?;
    let mut manifest = String::from("# KnowledgeOS 导出清单\n\n");
    for document in documents {
        manifest.push_str(&format!(
            "- {} ({})\n",
            document.title.unwrap_or(document.document_id),
...
fn resolve_document_for_execution(
    ...
    Ok(documents.into_iter().find(|document| {
        ...
        source_relative == normalized_target
            || normalized_relative.as_deref() == Some(normalized_target.as_str())
    }))
}
...
fn generate_markdown_content(
    ...
    let (prompt, context_blocks, metadata_json) = if let Some(source_document_id) =
        source_document_id
    {
        let loaded = if let Some(existing) = context.loaded_documents.get(source_document_id) {
            ...
            context.loaded_documents.insert(
                ...
                LoadedDocumentContext {
                    ...
                    title: document
                        .title
                        .unwrap_or_else(|| source_document_id.to_string()),
                    content_md,
                },
            );
            context
...
fn upsert_generated_markdown_document(
    ...
    if let Some(existing_document_id) = existing.as_ref().map(|item| item.0.as_str()) {
        let existing_document =
            get_document(&app_state.db, existing_document_id).map_err(|error| error.to_string())?;
        create_record_snapshot(
            &app_state.db,
            task_id,
            "document",
